...
app.get("/districts", async (req, res) => {
  // Try to enrich with ML risk_pct for the districts the model knows
  const mlRes = await callML("/ml/districts?state=Maharashtra");
  const mlMap = {};
  if (mlRes.ok && Array.isArray(mlRes.data)) {
    mlRes.data.forEach((d) => { mlMap[d.district.toLowerCase().trim()] = d; });
  }

  const data = maharashtraDistrictData.map((item) => {
