...
import { pctToSolidColor, pctToRiskLabel, buildDiseaseData, C } from "../utils/constants.js";
...
const DistrictOverlay = ({ district, detail, loading, onClose }) => {
  ...
  const diseases = {};
  const rawDiseases = detail?.diseases || buildDiseaseData(pct);

  Object.entries(rawDiseases).forEach(([key, d]) => {
    // If it has 'history' but not 'trend', map it
    if (d.history && !d.trend) {
      const last = d.history[d.history.length - 1];
      diseases[key] = {
        prevalence: last?.actual || 0,
        gap: (last?.expected || 0) - (last?.actual || 0),
        trend: d.history.map(h => ({ month: h.month, value: h.actual })),
      };
    } else {
      diseases[key] = d;
    }
  ...
  return (
    <>
      ...
      <div className="overlay-enter" onClick={e => e.stopPropagation()} style={{
        position: "fixed",
        inset: 0, margin: "auto",
        width: "min(86.4vw, 1062px)", height: "min(81vh, 684px)",
        ...
        <div style={{
          ...
          borderBottom:"1px solid #1a3a5c",background:"rgba(0,0,0,0.3)",padding:"0 8px",
        }}>
          {TABS.map(t => (
        ...
        {loading && (
          <div style={{
            ...
            <div style={{ width:38,height:38,border:"2px solid #1a3a5c",
