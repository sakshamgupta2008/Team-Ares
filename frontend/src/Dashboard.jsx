...
const C = { low: "#00ff9d", medium: "#ffb700", high: "#ff1a3c", default: "#0d2035", purple: "#bf5fff", cyan: "#00f5ff" };
...
function StatCard({ label, value, color, sub }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 9, color: "#7aa2c7", marginTop: 3,
        textTransform: "uppercase", letterSpacing: "0.1em" }}>{label}</div>
      {sub && <div style={{ fontSize: 9, color: "#a5c5e1", marginTop: 2 }}>{sub}</div>}
    </div>
  );
}
...
function Breadcrumb({ level, activeState, onGoCountry }) {
  return (
    <nav style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10,
      fontFamily: "'JetBrains Mono', monospace", color: "#4a7090" }}>
      <span
        onClick={level !== "country" ? onGoCountry : undefined}
        style={{
          color: level !== "country" ? "#00f5ff" : "#4a7090",
...
// ── Main Dashboard ────────────────────────────────────────────────────────
const Dashboard = () => {
  const [view,          setView]          = useState("map");      // "map" | "simulator" | "comparison"
  ...
  // ── Simulator / Comparison full-screen ────────────────────────────────
  if (view === "simulator") {
    return <InterventionSimulator onBack={() => setView("map")} />;
  }

  if (view === "comparison") {
    return <CityComparison onBack={() => setView("map")} />;
  }
    ...
    <div style={S.root}>
      ...
      <header style={S.header}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginLeft: "auto" }}>
          ...
          <button onClick={() => setView("comparison")} style={S.compareBtn}>
            ⚖ COMPARE CITIES
          </button>
...
const S = {
  ...
  compareBtn: {
    background: "rgba(191,95,255,0.08)", border: "1px solid rgba(191,95,255,0.35)",
    borderRadius: 8, padding: "7px 16px",
    color: "#bf5fff", fontSize: 10, fontWeight: 800,
    cursor: "pointer", fontFamily: "'JetBrains Mono', monospace",
    letterSpacing: "0.1em", boxShadow: "0 0 14px rgba(191,95,255,0.12)",
  },
